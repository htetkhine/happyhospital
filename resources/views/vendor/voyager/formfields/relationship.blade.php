@if(isset($options->model) && isset($options->type))
    ...
    @if(class_exists($options->model))
        ...
        @elseif($options->type == 'belongsToMany')
            ...
            @else
                @if($options->pivot_table == 'category_post') 
                    ...
                    @php
                        ...
                        foreach($relationshipOptions as $cat){

                            if($cat->parent_id != '' && $cat->parent_id > 0 ){
                                $catList[$cat->parent_id][] = $cat;
                            }else{
                                $catList['parent'][$cat->id] = $cat;
                            }       
                        }    
                    @endphp
             

                    @foreach($catList['parent'] as $relationshipOption)
                      <?php $parentid = $relationshipOption->{$options->key};?>
                        <div class="form-check">
                          <input class="form-check-input" type="checkbox" name="{{ $relationshipField }}[]" 
                              value="{{ $relationshipOption->{$options->key} }}" 
                              id={{ $relationshipOption->{$options->label} }}     
                              @if(in_array($relationshipOption->{$options->key}, $selected_values)) checked="checked" @endif
                           >
                            <label class="form-check-label" for={{ $relationshipOption->{$options->label} }}>
                              {{ $relationshipOption->{$options->label} }}
                            </label>
                        </div>
                        @if (isset($catList[$parentid] ))
                          @foreach($catList[$parentid] as $relationshipOption)
